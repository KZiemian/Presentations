...
% Number of chars: 62k+, 73k+, 25k+, 51k+, 71k+,
\RaggedRight
% ######################################
...
\begin{frame}
  ...
  % We need to stress that in his article Casimir provide physical
  % justification for presence of function $f$ and parameter $k_{ m }$.
  % Namely that metal is beckoning more transparent to electromagnetic
  % radiation, when energy of radiation grows.
...
\begin{frame}
  ...
  % interacting with two $\delta$-s. This model is well known in literature and is
  % in great details discussed in monograph by S. Albeverio et al.
...
\begin{frame}
  ...
  % which preclude computation of global energy by
...
\begin{frame}
  ...
  % Davide Fermi and Livio Pizzocchero found very obvious computation mistake
  % in it, information about it was published in
  % \textit{On~the~Casimir Effect with $\delta$-Like Potentials, and~a~Recent
  %   Paper by~K.~Ziemian (Ann. Herni Poincaré, 2021)}
  % \parencite{Fermi-Pizzocchero-On-the-Casimir-Effect-ETC-2023}.
  % Correction was issue by us in
