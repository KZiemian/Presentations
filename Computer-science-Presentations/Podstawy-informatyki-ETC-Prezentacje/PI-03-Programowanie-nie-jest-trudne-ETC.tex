...
% ######################################
\section{Abstrakcja i~typy zmiennych}
% ######################################
...
\begin{frame}
  ...
  termin na taką najmniejszą jednostkę pamięci to \textbf{słowo}
  (ang.~\textit{word}).
...
\begin{frame}
  ...
  By uczynić to zrozumiałym dla osób, które nigdy nie pisały w~assemblerze,
  przedstawimy teraz program „Hello, World!” napisany
  w~dialekcie~$\text{x}86$. Jest to dialekt assemblera stworzony przez
  firmę Intel i~wprowadzony do użycia w~$1972$ roku. Od tego czasu
  tej dialekt ten był używany na niezliczonej liczbie procesorów i~jego
  znaczenie dla świata trudno przecenić.

  Jeśli kod na następnej stronie jest dla Państwa odpychający, to znaczy,
  że~osiągnęliśmy nasz cel, wyjaśnienia czemu chcemy trzymać~się od takich
  rzeczy jak najdalej.
...
\begin{frame}
  ...
  Weźmy inny przykład. proszę sobie wyobrazić, że~zamiast zdjęciem na dysku,
  ...
  \textbf{abstrakcja}. Zamiast podawać bardzo abstrakcyjną~;) definicję
  tego terminu, odwołamy~się do przykładu.
  ...
  drapieżnikiem,\ldots” powiemy „To jest pies.”.
...
\begin{frame}
  ...
  Jak widzimy na przykładzie pojęcia „pies”, takie dzielenie rzeczy
  na różne kategorie i~operowanie tak wprowadzonymi kategoriami okazuje~się
  być bardzo wygodne nie tylko w~informatyce. Mamy nadzieję, że~takie
...
\begin{frame}
  ...
  Rozpatrzmy jeszcze jeden, mocno uproszczony przykład. Powiedzmy,
  że~używamy dwóch rodzajów plików: graficznych oraz tekstowych. Dzięki
  typom, nasz program może rozpoznać, że~chcemy zapisać coś do~pliku
  graficznego, ale otwarliśmy plik tekstowy, więc zgłosi on błąd, co jest
  bardzo pożądanym zachowaniem. Jeśli Państwo jeszcze nie rozumieją
  dlaczego otrzymywanie błędów jest dobrą rzeczą, to na tym przedmiocie
  napotkamy zapewne dostatecznie wiele przykładów, które pokażą, czemu
  tak jest.
  ...
  całkowitą~$1$ na liczbę zmiennoprzecinkową~$1.0$.
...
\begin{frame}
  ...
  Wedle innych języków, nie, bo~taka zmiana jednego typu liczbowego na
  drugi, może prowadzić do błędów numerycznych, których programista może
  nie być świadomy pisząc program. W~tym ostatnim typie~;) języków,
  programista musi jawnie poprosić o~\textbf{konwersje typów}. Czyli musi
  napisać kod, który mówi coś w~stylu „Wiem, że~to jest liczba całkowita
  i~w~tym miejscu masz ją zamienić na liczbę zmiennoprzecinkową. Ja biorę
  odpowiedzialność za to co~się stanie.” O~konwersji typów powiem w~swoim
  czasie.
