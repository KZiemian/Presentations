...
% Number of chars: 62k+, 73k+, 43k+, 52k+,
\RaggedRight
% ######################################
...
\begin{frame}
  ...
  wiedzą, czemu warto poświęcić swój czas. (Choć jak wiadomo, często potem
  stwierdzamy, że~nasz wybór mógł być jednak lepszy.)
  ...
  że~Państwo mają już wszystko umieć.
...
\begin{frame}
  ...
  assemblera sam w~sobie nas zupełnie nie interesuje?

  Jeśli jednak ktoś z~Państwa chce~się dowiedzieć więcej o~assemblerze,
...
% ##################
\begin{frame}
  \frametitle{Problemy z~assemblerem}
  ...
  \textbf{kompilatorem} tłumaczy na język assembler.
...
\begin{frame}
  ...
  \begin{figure}
    ...
    \begin{tikzpicture}
      ...
      \node[diagram square block] (C source code) at (0,0)
      {Kod programu w~języku C};

      \node[diagram block] (Compiler) at (0,-2.3) {Kompilator};

      \node[color=jNormalTextFGColor] (Magic) at (3,-2.3) {Magia};


      \draw[pointing arrow] (Magic) -- (Compiler);


      \draw[thick diagram arrow] (C source code) -- (Compiler);


      \node[diagram square block] (Assembly code) at (0,-4.6)
