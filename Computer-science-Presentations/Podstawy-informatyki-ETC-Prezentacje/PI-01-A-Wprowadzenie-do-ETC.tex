...
\subtitle{Spotkanie~1,~A. Wprowadzenie do~kursu}
...
\begin{frame}
  ...
  adres \email. W~tytule maila proszę napisać „Prośba o~konsultacje” lub
...
\begin{frame}
  ...
  „Pytanie o~zaliczenie zaoczne”, inaczej mail może na długo zaginąć
  wśród maili, które nie wymagają szybkiej odpowiedzi.
...
\begin{frame}
  ...
  lub coś podobnego, bo inaczej na długo zaginą wśród innych.
...
% ##################
\begin{frame}
  \frametitle{Odnośnie rozwiązywań zadań}
  ...
  prostych zadań, co jest zupełnie zbędne. Oczywiście, nikt nie straci
  punktów za oddanie skomplikowanego programu, ale jest wiele powodów, by
  poprzestać na pisaniu prostego kodu. Po pierwsze, to zwykle oszczędza
  czas spędzony na~rozwiązywaniu zadań na~ten przedmiot. Po~drugie,
  prostota i~czytelność kodu są naprawdę ważne przy tworzeniu programów na
  każdym poziomie.
...
\begin{frame}
  ...
  tylko niemu rozszerzenie Pliki napisane w~języku~C mają mieć
  rozszerzenie~\texttt{.c}. Analogicznie, każdy program w~języku Python
  powinien~się kończyć na~\texttt{.py}.
...
% ##################
\begin{frame}
  \frametitle{Nazwy plików}


  Niekiedy ktoś nazwie swój plik jako \\
  \texttt{plik-numer.1.c} \\
  co jest proszeniem~się o~problem. Programy mogą błędnie rozpoznać
  „\texttt{.1.c}” jako rozszerzenie pliku i~wtedy mogą~się dziać różne
  nieporządne rzeczy.
...
\begin{frame}
  ...
  według indeksu \textsc{tiobe}, poczytać o~każdym z~nich i~wybrać ten
  który Państwo uważają za~najlepszy.
...
\begin{frame}
  ...
  W~grudniu $2024$ roku indeks \textsc{tiobe} przedstawia~się w~następujący
  sposób: Python, C++, Java, C, C\#, JavaScript, Go, \textsc{sql}, Visual
  Basic, Fortran, Delphi/Object Pascal, \textsc{php}, Scratch, Rust,
  \textsc{matlab}, R, język asemblera, Ruby, \textsc{cobol}, Swift.
