...
\begin{frame}
  ...
  taki jak ten, jest ważne dla zrozumienia jak ten algorytm działa
...
\begin{frame}
  ...
  do~\texttt{last - 1 := 0} porównując \texttt{A[j]}
  z~\texttt{tempPivot := 7}. \\
  \texttt{A[0] := 1 <= tempPivot}, więc zamieniamy \texttt{A[0]}
...
\begin{frame}
  ...
  4.~Zamieniamy miejscami \texttt{A[i] = A[1]} i~\texttt{A[last] = A[1]}
  (czyli znowu nic nie robimy): \\
  \texttt{A := \{1, 7\} } \\
  Oś obrotu jest teraz równa obecnej wartości zmiennej~\texttt{i}: \\
  \texttt{p := i := 1}.

  5.~Dzielimy tablicę \texttt{A} na dwie części tablic: \\
  \texttt{B := \{1\} }, \texttt{first := 0},
  \texttt{last := 1 - 1 := 0}, \\
  \texttt{C := \{\} }, \texttt{first := 1 + 1 := 2},
  \texttt{last := 1}. Tablica ta jest pusta, bo jej początkowy indeks jest
  większy od jej indeksu końcowego.
...
\begin{frame}
  ...
  więc do punktu~$1$. Przyjmujemy przy tym, że~każda zmiana
  ...
  1,~B).~Tablica \texttt{B := \{1\}} jest jednoelementowa, więc
  ...
  spełniony warunek \texttt{first >= last} (\texttt{2 >= 1}).

  W~tej chwili \texttt{A := \{1, 7\}} więc tablica nie uległą zmianie
  i~jest posortowana.
...
\begin{frame}
  ...
  4.~Zamieniamy miejscami \texttt{A[i] = A[0]} i~\texttt{A[last] = A[1]}: \\
  \texttt{A := \{3, 8\} } \\
  Oś obrotu jest teraz równa obecnej wartości zmiennej~\texttt{i}: \\
  \texttt{p := i := 0}.

  5.~Dzielimy tablicę \texttt{A} na dwie części tablic: \\
  \texttt{B := \{3\} }, \texttt{first := 0},
  \texttt{last := 1 - 0 := 1}, \\
  \texttt{C := \{8\} }, \texttt{first := 0 + 1 := 1}, \texttt{last := 1}.
...
\begin{frame}
  ...
  1.~Tablica \texttt{B := \{3\}} jest jednoelementowa, więc
  z~poprzedniego przykładu wiemy, że~algorytm zachowa~się poprawie
  i~zostawi tą tablicę niezmienioną. To~samo jest prawdą dla tablicy
  \texttt{C := \{8\}}.

  W~tej chwili \texttt{A := \{3, 8\}}, więc tablica ta jest posortowana
  w~sposób rosnący.
...
% ##################
\begin{frame}
  \frametitle{Komentarz do przykładu~4}
