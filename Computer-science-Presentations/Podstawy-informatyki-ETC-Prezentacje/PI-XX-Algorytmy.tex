...
\begin{frame}
  ...
  % te zaś większe od \texttt{tempPivot} na prawej stronie \texttt{p := 3}.
...
\begin{frame}
  ...
  % 4.~Zamieniamy miejscami elementy \texttt{A[i] = A[4]} i~\texttt{A[last] = A[9]}: \\
...
\begin{frame}
  ...
  % 2B.~Przechodzimy z~wartością zmiennej \texttt{j}
  % od~\texttt{j := newFirst1 := 0} \\
  % do~\texttt{j := newLast - 1 := 1} porównując \texttt{A[j]}
  % z~\texttt{tempPivot := 7}. \\
  ...
  % 1B.~\texttt{B = \{1, 0, 3\}}, \texttt{newFrist1 := 0},
  % \texttt{newLast1 := 2}. Nie jest spełniony żaden z~warunków
  % \texttt{newFirst1 >= newLast1} (\texttt{0 >= 2}),
  % \texttt{newFirst1 < 0} (\texttt{0 < 0}), więc przystępujemy do obliczenia
  % osi obrotu.
