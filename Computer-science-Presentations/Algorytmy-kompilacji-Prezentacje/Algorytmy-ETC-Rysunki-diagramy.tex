...
% For drawing bits and bytes in PGF/TikZ
\usepackage{./Local-packages/PGF-TikZ-Graphs-styles}
...
%       \node[diagram square block] at (0,0) {Czymś jest};
...
%   w~jednym pliku napisany w~języku assembler. Proces uruchamiania tej
...
%       \node[diagram square block] (Hollow Knight) at (0,0)
%       {„Hollow Knight” (język C++)};
...
% % ##################
% \begin{frame}
%   \frametitle{Jeśli myślisz, że~jesteś dobrym programistą???}
...
%       \node[color=jNormalTextFGColor,align=left] at (3.2,-1.9)
%       {Przesył danych między \\
%         pamięcią i~procesorem};

% % ##################
% % !!!!!!!!!!!!!!!!!!!!
% \begin{frame}
%   \frametitle{Architektura von Neumanna}
...
%       % Połączenie procesora i pamięci
%       \draw[thick diagram arrow] (-1,-1.25) -- (-1,-2.47);

%       \draw[thick diagram arrow] (1,-2.47) -- (1,-1.25);
...
%       % Pamięć
%       \fill[color=brown] (-3,-1.25) rectangle (3,1.25);

%       \node[color=jMathTextFGColorOfStyleDark] at (0,0) {Pamięć};



%       % Wejście
%       \draw[thick diagram arrow] (-4.75,0) -- (-3,0);

%       \node at (-4.175,0.35) {Wejście};


%       % Wyjście
%       \draw[thick diagram arrow] (3,0) -- (4.75,0);

%       \node at (3.825,0.35) {Wyjście};



%       % Procesor, CPU
%       \node[diagram block] at (0,-3.25) {Procesor};

%     \end{tikzpicture}

%     \caption{Architektura von Neumanna}
...
%       \node[diagram block] (Poczatek programu) at (0,0)
%       {Start \\
%         programu};

%       \node[diagram block] (Magia) at (0,-2.3) {Magia};


%       \draw[thick diagram arrow] (Poczatek programu) -- (Magia);


%       \node[diagram block] (Koniec programu) at (0,-4.6)
%       {Koniec \\
%         programu};


%       \draw[thick diagram arrow] (Magia) -- (Koniec programu);

%     \end{tikzpicture}

%     \caption{Opis działania typowego kodu}
...
\begin{frame}
  ...
  \begin{figure}
    ...
    \caption{Ilustracja podziału na fazę kompilacji i~wykonania
      danego programu}
...
%   \tikz{ \node[bit node] (bit 0 power) at (0,0) {1}; }
%   \tikz{ \node[bit node] (bit 0 power) at (0,0) {0}; }

%   \begin{tikzpicture}
%     [node distance=\bitsDistance]

%     \node[bit node] (bit 0 power) at (0,0) {$1$};

%     \node[bit node,left=of bit 0 power] (bit 1 power) {$0$};

%     \node[bit node,right=of bit 0 power] {$.$};

%     \node[special bit node,left=of bit 1 power] (bit 2 power) {$1$};

%     \node[scale=\nodeScaleBigger,left=of bit 2 power] {$2 =$};
