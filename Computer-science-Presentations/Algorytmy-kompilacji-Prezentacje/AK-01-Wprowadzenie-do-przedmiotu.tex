...
\begin{frame}
  ...
  Ze względu na ramy czasowe wiele rzeczy będę \alert{upraszczał} lub
  \alet{pomijał}. Proszę o~tym pamiętać podczas uczęszczania na zajęcia
...
% ##################
\begin{frame}
  \frametitle{Bardzo ważna problem}
...
\begin{frame}
  ...
  czy x86/Intel. Typ assemblera jest ustalony przez firmę, która
  ...
  napisanemu w~dwóch??? rodzajach assemblera, językach C i~Python.
...
% ##################
\begin{frame}
  \frametitle{„Hello, World!” w~assemblerze ARM 32,
...
\begin{frame}
  ...
  Na pierwszy rzut oka powinno być jasne, że~C jest prostszy od~assemblera,
  ...
  Jak rozwiązujemy ten problem dziś? Tworzymy języki takie jak~C czy
  Python, piszemy w~nich programy, które następnie odpowiedni program zwany
  \textbf{translator} tłumaczy na język assembler.
...
% ##################
\begin{frame}
  \frametitle{O~assemblerze słów kilka}


  Jak mówiliśmy wcześniej, na potrzeby tego kursu będziemy przyjmować,
  że~naturalnym językiem komputera jest język assembler i~nie będziemy
  ...
  \textbf{język assemblera} (ang. \textit{assembly langauge})
  i~\textbf{program assembler} (ang. \textit{assembler}), który operuje
  na~programie napisany w~języku assembler. W~języku polskim „assembler”
  oznacza często język assembler, co może powodować pewne nieporozumienia.
...
% ##################
\begin{frame}
  \frametitle{Pisanie w~języku assembler?}


  Aby opisać jak z~grubsza wygląda wykonanie dowolnego programu muszę
  wprowadzić pewną dozę uproszczeń. Przyjmijmy po pierwsze, że~gra „Hollow
  Knight” (nie gorszy przykład programu niż dowolny inny), zawiera~się
  w~jednym pliku napisany w~języku assembler.
