...
\begin{frame}
  ...
  % Tutaj pojawia~się pewien problem. Zilustrujemy go pokazują program
  % „Hello, World!” napisany w~dialektach asemblera napisanemu w~dwóch???
  % rodzajach asemblera oraz językach C i~Python.


  % Dobrze, to
  % w~takim razie ja możemy uruchomić program napisany w~języku~C czy Python?

  % Wszystkie inne
  % języki (C, C++, Java, Python, etc.), są dla niego niezrozumiałe. Z~tego
  % powodu program napisany w~tych językach muszą zostać najpierw
  % \alert{przetłumaczone} do jego
