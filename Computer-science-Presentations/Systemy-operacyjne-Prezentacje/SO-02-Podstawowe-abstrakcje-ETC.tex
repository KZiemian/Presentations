...
\subtitle{Podstawowe abstrakcje, korzystanie z~powłoki
  \textsc{bash}, etc.
}
...
\begin{frame}
  ...
  Gilesa Orra
  \colorhref{https://tldp.org/HOWTO/Bash-Prompt-HOWTO/index.html}
  {\textit{Bash Prompt HOWTO}} \parencite{Orr-Bash-Prompt-HOWTO-Ver-2003}.
...
% ######################################
\section{Podstawy poruszania~się w~powłoce BASH}
% ######################################
...
\begin{frame}
  ...
  \texttt{adam:$\sim$\$} \\
  Od tego momentu będziemy skracali ten znak zachęty (ang. \textit{prompt})
  do symbolu \texttt{\$}. Przepisywanie pełnej jego postaci wnosi bardzo
  mało informacji, za~to zabiera sporo miejsca.
...
\begin{frame}
  ...
  Zacznijmy od~polecenia \texttt{ls}, od ang. \textit{List fileS}, które
  wyświetla zawartość katalogu w~którym~się znajdujemy. Z~tego powodu
  wynik działania tego polecenia będzie~się cały czas zmieniać, jednym
  z~możliwych może być ten przedstawiony poniżej. \\
  \texttt{\$ ls} \\
  \texttt{{\color{jAxisBlue} Dokumenty}} \\
  \texttt{{\color{jAxisBlue} Pobrane}} \\
  \texttt{introduction-to-GNU-linux.pdf}
