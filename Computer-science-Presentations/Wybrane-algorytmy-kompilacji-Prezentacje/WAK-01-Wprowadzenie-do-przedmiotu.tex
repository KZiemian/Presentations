...
\begin{frame}
  ...
  i~towarzyszące im materiały były możliwie proste, łatwe w~zrozumieniu
...
\begin{frame}
  ...
  wejściowych, tworząc obiekty wyjściowe. Takim obiektem może być kucharz,
  komputer, program komputerowy,~etc.
...
\begin{frame}
  ...
  przedmiot, acz o~kilku powodach wspomnimy potem.
...
\begin{frame}
  ...
  w~użyciu, niż dany dialekt asemblera i~piszemy program w~takim właśnie
...
\begin{frame}
  ...
  równoważne słowom „kod” i~„program”, choć intuicyjnie przez „program”
...
% ######################################
\section{Działanie interpreterów i~kompilatorów}
% ######################################
...
\begin{frame}
  ...
  wykonywania żadnej z~podanych instrukcji. To jak przeczytanie
  przepisu kucharskiego, ale bez wykonania zawartym w~nim instrukcji, takich
...
\begin{frame}
  ...
  3) Wiedząc, że~chcemy obliczyć sumę $1 + 2 + \ldots + 100$ kompilator korzysta
  ze wzoru: \\[0.3em]
...
\begin{frame}
  ...
  jak działa interpreter i~kompilator, ale już on pozwala zrozumieć kilka
  ważnych rzeczy.

  Przede wszystkim kompilator korzysta z~zasady czarnej skrzynki. Nie
  interesuje nas jak będzie działa program wytworzony przez kompilator,
  jeśli będzie dawał poprawny wynik i~będzie działa możliwie szybko.

  Nie jest więc ważne, czy program pracowicie będzie liczył
  $1 + 2 + 3 + \ldots$, czy wyliczy $100 \cdot 101 / 2$.
  Z~naszego punktu widzenia w~środku programu może być smok, który wykonuje
  odpowiednie obliczenia matematyczne, byleby tylko ten smok zwracał
  poprawną odpowiedź i~to możliwie szybko.
...
\begin{frame}
  ...
  gdy chodzi o~język „silnie” kompilowany. Mnie tak nauczono myśleć
...
\begin{frame}
  ...
  Teraz przejdziemy do kilku praktycznych kwestii, które pozwoli nam
  zilustrować poprzedni przykład.
