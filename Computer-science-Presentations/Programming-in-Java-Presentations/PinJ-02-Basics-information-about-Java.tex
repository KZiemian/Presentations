...
\begin{frame}
  ...
  We enter this directory and create file \texttt{PinJ-01.java}. \\
  ...
  Java files, at an appropriate time.
...
\begin{frame}
  ...
  inside it, what we should expect: ``Hello, World!'' program.
  ...
  We now compile it using \\
  \texttt{\$ javac PinJ-01.java} \\
  and then run it \\
  \texttt{\$ java Main} \\
  If everything run smoothly, we will see on the screen \\
  \texttt{Hello, World!}
...
\begin{frame}
  ...
  feature of Java is that as of today, \currentYear, four other version of
  Java are still supported, which is a~big number for a~language.
...
\begin{frame}
  ...
  {Sun Microsystems} and this company holds legal control over Java
  programming language. In $2010$ Sun was acquired by
  \colorhref{https://en.wikipedia.org/wiki/Oracle\_Corporation}
  {Oracle Corporation} and this company now hold rights to Java.
  It must be noted that Oracle have in many circles very bad reputation,
  for using it position and rights to disadvantages of computer science
  community. This of course also is true for Java.

  We also should note that the situation where one company holds rights
  to programming language is not the norm. Many languages are in public
  domain, other are properties of companies, you should always check
  what is the status of your language of interest.
...
\begin{frame}
  ...
  popularity of programming languages. It is worth looking at top $10$
...
\begin{frame}
  ...
  time in USA everything related to the Internet was easy to sell and Java
  language was among one of the~hottest things that year. Remember
  that Java was ``the programming language of the~Internet''. According to
  Eich, Netscape decided to include the word ``Java'' in the~name
  ``JavaScript'', to get a~free advertisement. So, the~name ``JavaScript''
  is basically a~gimmick, a~marketing ploy.
...
\begin{frame}
  ...
  definition, but for us it is good enough.
...
\begin{frame}
  ...
  feature from Java $17$ in version~$8$ everything will crash and other
  ...
  a~good programmer wrote it around $2005$ and is still working? So why
  should we update it?
...
\begin{frame}
  ...
  course will give you solid foundations for learning deeper layers of Java
  and things like``How to handle different versions of Java?''. We~also
  hope that you understand that learning core of Java is already a~big task
  and going beyond that is already too challenging for this particular
  course.

  In the past Java was infamous for its slowness, but things have changed
  a~lot since then and currently, new versions of Java (too many versions
  ;)) are among the fastest languages on the market.
...
\begin{frame}
  ...
  we can write the code. To put some order in it,
...
\begin{frame}
  ...
  We shouldn't waste our time arguing ``Is language X is object-oriented
...
\begin{frame}
  ...
  done for~pedagogical reasons, which should became clear when we advanced
...
\begin{frame}
  ...
  according to this idea. Let us see some examples.  In other words, class
  is an abstract idea that describes what a given object is and how it
  behaves. Instance of the class is concrete object (we all full aware that
  we use word ``object'') built according to this idea. Let us see some
  examples.
...
\begin{frame}
  ...
  value to a~new part of memory and now both users have its own copy of this
  important number. This is called \textbf{passing by value}.
...
%   In language as C and C++ addresses of object are quite easy to get.
%   In Java we also can get addresses of our objects, but we should avoid
%   doing that. Java was built in such way, to minimize.
